			...
			<div class="col-md-8" style="padding:5px 5px 5px 5px;border-radius: 4px;">
				<div class="col-md-12" style="background-color:white; padding:5px 5px 5px 5px;border-radius: 4px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
					...
					<?php	
						...
						$result = mysql_query("select * from kerala LIMIT $start, $limit");       
							if($result){				
								while($row=mysql_fetch_array($result)){	
									...
									echo  '<p align="right"><a href="kerala_article.php?articleid='.$articleID.'" class="btn btn-info" role="button">View More</a></p></div>';
									echo '</div>';
									echo '</div>';
									echo '<div><hr></div>';
									}}	
					...
					<div class='col-md-12'>
							...
							$rows=mysql_num_rows(mysql_query("select * from kerala"));
