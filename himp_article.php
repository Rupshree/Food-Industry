...
<?php
	...
	$maxid = mysql_query("Select max(id) from himp") or die(mysql_error());
	$max = mysql_fetch_array($maxid);

	if(!empty($articleID)){
		$sqlSelectSpecProd = mysql_query("select * from himp where id = '$articleID'") or die(mysql_error());
	...
	<div class="container">
		...
		<div class="well" style="min-height:45px; padding:5px; margin-bottom:10px;">
			...
			<div class="col-md-12">	
					...
					<?php
					if($articleID > 1){
					echo  '<div class="col-md-3" aline="left"><p><a href="himp_article.php?articleid='.--$articleID.'" aline="left" class="btn btn-primary" role="button">Pre</a></p></div>';
					?>
					...
					if($articleID != $max[0]){
					echo  '<div class="col-md-8"></div><div class="3" aline="right"><p><a href="himp_article.php?articleid='.++$articleID.'" aline="right" class="btn btn-primary" role="button">Next</a></p></div>';
					}
					}
					else{
						echo  '<div class="col-md-11"></div><div class="3" aline="right"><p><a href="himp_article.php?articleid='.++$articleID.'" aline="right" class="btn btn-primary" role="button">Next</a></p></div>';
						/*echo 'max($articleID)';*/
					}
					?>
